...
export default {
  getFeaturedData() {
    return async(apiUrl + '/recommendPage/nodes/5910018c8094b1e228e5868f').then(
      data => data.data
    )
  },
  ...
  getSwiperPictures() {
    return async(
      apiUrl + '/recommendPage/node/spread/575f74f27a4a60dc78a435a3',
      {
        pl: 'ios'
      }
    ).then(data => data.data)
  ...
  getBookList(id, st = 1, size = 10) {
    return async(
      apiUrl + '/recommendPage/node/books/all/' + id,
      {
        ajax: 'ajax',
        st: st,
        size: size
  ...
  getSearchHotKeywords() {
    return async(apiUrl + '/book/search-hotwords').then(
      data => data.searchHotWords
    )
  },
